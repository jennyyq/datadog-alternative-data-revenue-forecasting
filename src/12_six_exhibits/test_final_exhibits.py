...
def test_exhibit4_reflects_current_walkforward_csv_not_stale_75pct():
    import os, tempfile
    wf = pd.read_csv("outputs/walkforward_results.csv")
    m = wf[(wf.panel == "main") & (wf.model.isin(F.MODEL_ORDER))].set_index("model")
    assert len(m) == len(F.MODEL_ORDER)
    with tempfile.TemporaryDirectory() as tmp:
        path=os.path.join(tmp,"e4.png")
        out=F.exhibit4_walkforward(wf,path)
