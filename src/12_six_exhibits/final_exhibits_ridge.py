...
Outputs (all PNG, outputs/):
  ...
  exhibit4_walkforward_comparison.png    - 8-model OOS comparison (error metrics + directional accuracy)
...
MODEL_ORDER = ["Naive", "AR1", "Cloud", "Trends", "Cloud+Trends", "AR+Cloud+Trends",
               "Ridge_Cloud+Trends", "Ridge_AR+Cloud+Trends"]
MODEL_LABEL = {"Naive": "Naive", "AR1": "AR(1)", "Cloud": "Cloud", "Trends": "Trends",
               "Cloud+Trends": "Cloud + Trends", "AR+Cloud+Trends": "AR + Cloud + Trends",
               "Ridge_Cloud+Trends": "Ridge · Cloud + Trends",
               "Ridge_AR+Cloud+Trends": "Ridge · AR + Cloud + Trends"}


def combine_walkforward_results(ols, ridge):
    """Combine independently generated main-panel results; do not refit or alter forecasts."""
    required = {"panel", "model", "n_oos", "MAE", "RMSE", "MAPE_pct", "Directional_Accuracy_pct"}
    for name, frame in (("OLS", ols), ("Ridge", ridge)):
        if not required.issubset(frame.columns):
            raise ValueError(f"{name} missing columns: {sorted(required - set(frame.columns))}")
    combined = pd.concat([ols, ridge], ignore_index=True)
    main = combined.loc[combined.panel == "main"].copy()
    duplicates = main.model[main.model.duplicated()].tolist()
    if duplicates:
        raise ValueError(f"Duplicate main-panel models: {duplicates}")
    missing = set(MODEL_ORDER) - set(main.model)
    if missing:
        raise ValueError(f"Missing main-panel model(s): {sorted(missing)}")
    main = main.set_index("model").loc[MODEL_ORDER]
    if main.n_oos.nunique() != 1 or int(main.n_oos.iloc[0]) != 8:
        raise ValueError("All eight models must use the same eight OOS quarters")
    if not np.isfinite(main[["MAE", "RMSE", "MAPE_pct", "Directional_Accuracy_pct"]].to_numpy(dtype=float)).all():
        raise ValueError("Non-finite model metrics")
    return main


def exhibit4_walkforward(ols, ridge, path):
    """Eight-model comparison from current CSVs, with no hard-coded metric results."""
    m = combine_walkforward_results(ols, ridge)
    eligible = m.drop(index="Naive")  # persistence does not forecast a directional turn
    most_direction = eligible.Directional_Accuracy_pct.max()
    direction_models = eligible.index[np.isclose(eligible.Directional_Accuracy_pct, most_direction)].tolist()
    min_errors = {c: float(m[c].min()) for c in ("MAE", "RMSE", "MAPE_pct")}
    lowest_mae = m.MAE.idxmin()
    fig, ax = plt.subplots(figsize=(15, 8.8))
    fig.patch.set_facecolor("white")
    ax.set_axis_off(); ax.set_xlim(0, 1); ax.set_ylim(0, 1)
    fig.text(.065, .948, "EXHIBIT 04   /   FORECAST VALIDATION", color=C_PURPLE, fontsize=11, weight="bold")
    fig.text(.065, .899, "Out-of-sample model comparison", fontsize=23, weight="bold", color="#182336")
    fig.text(.065, .859, "Revenue YoY  ·  2024 Q3–2026 Q2  ·  8 expanding-window forecasts  ·  OLS versus Ridge (α = 10)",
             fontsize=11, color="#617083")
    xs = [.05, .48, .625, .755, .89]
    for x, lbl in zip(xs, ["MODEL", "MAE ↓", "RMSE ↓", "MAPE ↓", "DIRECTION ↑"]):
        ax.text(x,.775,lbl,transform=ax.transAxes,va="center",ha="left" if x==xs[0] else "center",
                fontsize=11,weight="bold",color="#5a6a7b")
    ax.plot([.04,.965],[.741,.741],transform=ax.transAxes,color="#cbd5e1",lw=1.4)
    for i, model in enumerate(MODEL_ORDER):
        y=.693-i*.072
        if i%2==0:
            ax.add_patch(plt.Rectangle((.04,y-.032),.925,.064,transform=ax.transAxes,
                                       facecolor="#f2f6fb",edgecolor="none"))
        if i==6:
            ax.plot([.04,.965],[y+.039,y+.039],transform=ax.transAxes,color="#cbd5e1",lw=.9)
        row=m.loc[model]
        ax.text(xs[0],y,MODEL_LABEL[model],transform=ax.transAxes,va="center",fontsize=11.6,
                color="#17253a",weight="bold" if model==lowest_mae or model in direction_models else "normal")
        vals=[float(row.MAE),float(row.RMSE),float(row.MAPE_pct),float(row.Directional_Accuracy_pct)]
        for j,(value,fmt) in enumerate(zip(vals,["{:.2f}","{:.2f}","{:.1f}%","{:.1f}%"])):
            hit=(j<3 and np.isclose(value,min_errors[("MAE","RMSE","MAPE_pct")[j]])) or (j==3 and model in direction_models)
            if hit:
                ax.add_patch(FancyBboxPatch((xs[j+1]-.045,y-.025),.09,.05,transform=ax.transAxes,
                              boxstyle="round,pad=.003,rounding_size=.01",facecolor="#fff0dc" if j<3 else "#dcf4e8",
                              edgecolor="none"))
            ax.text(xs[j+1],y,fmt.format(value),transform=ax.transAxes,va="center",ha="center",fontsize=11.5,
                    weight="bold" if hit else "normal",color=("#a94d04" if j<3 else "#166b49") if hit else "#263449")
    ax.plot([.04,.965],[.092,.092],transform=ax.transAxes,color="#cbd5e1",lw=1)
    fig.text(.065,.118, f"Lowest MAE: {MODEL_LABEL[lowest_mae]}  ·  {m.loc[lowest_mae,'MAE']:.2f} pp",fontsize=11.3,weight="bold",color="#a94d04")
    fig.text(.065,.078, f"Direction calls: {', '.join(MODEL_LABEL[x] for x in direction_models)}  ·  {most_direction:.1f}%",fontsize=10.7,weight="bold",color="#166b49")
    fig.text(.065,.043,"Naive's 0% direction score is mechanical. Ridge α=10 is exploratory; post-2022 robustness does not establish a consistent gain.",
             fontsize=9.6,color="#617083")
    fig.savefig(path,dpi=175,facecolor="white",bbox_inches="tight");plt.close(fig)
    return {"lowest_mae_model":lowest_mae,"highest_direction_models":direction_models,
            "highest_direction_pct":float(most_direction),"model_count":len(m)}
...
def main():
    ...
    ridge_path = os.path.join(IN_DIR, "walkforward_ridge_results.csv")
    if not os.path.isfile(ridge_path):
        raise FileNotFoundError(f"Ridge results missing: {ridge_path}. Run walkforward_ridge_addon.py first.")
    wf_ridge = pd.read_csv(ridge_path)
    ...
    exhibit4_walkforward(wf_main, wf_ridge, OUT_DIR + "exhibit4_walkforward_comparison.png")
